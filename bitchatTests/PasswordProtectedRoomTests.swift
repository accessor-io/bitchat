...
class PasswordProtectedRoomTests: XCTestCase {
    ...
    override func setUp() {
        ...
        // Create a fresh view model and encryption service for each test
        encryptionService = EncryptionService()
        viewModel = ChatViewModel(encryptionService: encryptionService)
    ...
    func testPasswordKeyDerivation() {
        ...
        let key1 = encryptionService.deriveRoomKey(password: password, roomName: roomName)
        let key2 = encryptionService.deriveRoomKey(password: password, roomName: roomName)
    ...
    func testDifferentPasswordsProduceDifferentKeys() {
        ...
        let key1 = encryptionService.deriveRoomKey(password: password1, roomName: roomName)
        let key2 = encryptionService.deriveRoomKey(password: password2, roomName: roomName)
    ...
    func testDifferentRoomsProduceDifferentKeys() {
        ...
        let key1 = encryptionService.deriveRoomKey(password: password, roomName: room1)
        let key2 = encryptionService.deriveRoomKey(password: password, roomName: room2)
    ...
    func testJoinPasswordProtectedRoomWithMessages() {
        ...
        do {
            let encryptedData = try encryptionService.encrypt(data: messageData, key: key)
    ...
    func testEncryptDecryptRoomMessage() {
        ...
        // Derive key
        let key = encryptionService.deriveRoomKey(password: password, roomName: roomName)
    ...
    func testWrongPasswordFailsDecryption() {
        ...
        let correctKey = encryptionService.deriveRoomKey(password: correctPassword, roomName: roomName)
        ...
        do {
            let encryptedData = try encryptionService.encrypt(data: messageData, key: correctKey)

            // Try to decrypt with wrong password
            let wrongKey = encryptionService.deriveRoomKey(password: wrongPassword, roomName: roomName)
...
extension PasswordProtectedRoomTests {
    // Helper method to derive room key for testing using EncryptionService
    func deriveRoomKey(from password: String, roomName: String) -> SymmetricKey {
        return encryptionService.deriveRoomKey(password: password, roomName: roomName)
    }
}